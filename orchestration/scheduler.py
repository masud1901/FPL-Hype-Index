...
class DataCollectionScheduler:
    ...
    async def _run_daily_scrapers(self):
        ...
        try:
            ...
            tasks = [
                self.coordinator.run_scraper('fpl_api')
            ]
    ...
    async def _run_weekly_scrapers(self):
        ...
        try:
            ...
            tasks = [
                ...
                # self.coordinator.run_scraper('understat'),  # Will be added later
                # self.coordinator.run_scraper('fbref'),      # Will be added later
            ]
            ...
            # Log results
            scrapers = ['fpl_api']  # Add more as they're implemented
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    self.logger.error(
                        "Weekly scraper failed",
                        scraper=scrapers[i],
                        error=str(result)
                    )
                else:
                    self.logger.info(
                        "Weekly scraper completed",
                        scraper=scrapers[i],
                        result=result
                    )
    ...
    async def _run_biweekly_scrapers(self):
        ...
        try:
            ...
            tasks = [
                # self.coordinator.run_scraper('transfermarkt'),  # Will be added later
                # self.coordinator.run_scraper('whoscored'),      # Will be added later
            ]
            ...
            # Log results
            scrapers = []  # Add scrapers as they're implemented
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    self.logger.error(
    ...
    async def _run_live_data_scrapers(self):
        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    self.logger.error(
                        "Live data scraper failed",
                        scraper="fpl_api",
                        error=str(result)
                    )
                else:
                    self.logger.info(
                        "Live data scraper completed",
                        scraper="fpl_api",
                        result=result
                    )
