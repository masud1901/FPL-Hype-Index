...
class DataCoordinator:
    ...
    async def run_scraper(self, scraper_name: str, config_overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    async def run_multiple_scrapers(self, scraper_names: List[str], config_overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(
                f"Starting multiple scraper execution",
                scrapers=scraper_names
            )
            
            # Create tasks for all scrapers
            tasks = [
                self.run_scraper(name, config_overrides)
            ...
            self.logger.info(
                ...
                successful=len([r for r in final_results.values() if r.get("status") == "success"]),
                failed=len([r for r in final_results.values() if r.get("status") == "failed"])
            )
    ...
    async def _handle_scraper_error(self, scraper_name: str, error: ScraperException, start_time: datetime):
    ...
    async def _handle_general_error(self, scraper_name: str, error: Exception, start_time: datetime):
        ...
        try:
            with db_manager.get_sync_session() as session:
                ...
                if last_run:
                    return {
                        'scraper': scraper_name,
                        'last_run': last_run.start_time.isoformat() if last_run.start_time else None,
