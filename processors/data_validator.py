...
class DataValidator:
    ...
    def _validate_players_data(self, players: List[Dict[str, Any]]) -> List[str]:
        ...
        required_player_fields = ['id', 'first_name', 'second_name', 'team', 'element_type']
        ...
        for i, player in enumerate(players):
            ...
            if 'element_type' in player:
                ...
                if not isinstance(element_type, int) or element_type not in [1, 2, 3, 4]:
                    issues.append(f"Player {i} has invalid element_type: {element_type}")
            
            if 'now_cost' in player:
                cost = player['now_cost']
                if not isinstance(cost, int) or cost < 0 or cost > 1500:  # Max reasonable price
    ...
    def _validate_gameweeks_data(self, gameweeks: List[Dict[str, Any]]) -> List[str]:
        ...
        required_gameweek_fields = ['id', 'name', 'deadline_time', 'finished', 'data_checked']
        ...
        for i, gameweek in enumerate(gameweeks):
            ...
            if 'id' in gameweek and not isinstance(gameweek['id'], int):
                issues.append(f"Gameweek {i} has invalid ID type: {type(gameweek['id'])}")
            
            if 'finished' in gameweek and not isinstance(gameweek['finished'], bool):
                issues.append(f"Gameweek {i} has invalid finished type: {type(gameweek['finished'])}")
