...
from .models import FPLPlayer, FPLTeam, FPLGameweek, FPLBootstrapData, FPLScrapedData
...
class FPLScraper(BaseScraper):
    ...
    def validate_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not players:
                self.logger.logger.warning(
                    "No players data found in FPL response",
                    scraper=self.name
                )
                return False
            ...
            if not teams:
                self.logger.logger.warning(
                    "No teams data found in FPL response",
                    scraper=self.name
                )
                return False
            ...
            if not gameweeks:
                self.logger.logger.warning(
                    "No gameweeks data found in FPL response",
                    scraper=self.name
                )
                return False
            ...
            self.logger.logger.error(
                "Error during FPL data validation",
                scraper=self.name,
                error=str(e)
            )
            return False
